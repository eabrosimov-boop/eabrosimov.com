...
**Прошедшие даты убираются автоматически** — выезд исчезает со сайта на следующий день после `to`.
Фильтр сидит в двух местах: `upcomingDates()` в main.js (главная) и `datesHtml()` в generate-tours.js (страницы туров).
Старые даты из TOUR_DATES **удалять не нужно** — они остаются историей. Если будущих дат не осталось, блок «Доступные даты» на странице тура не выводится вовсе.
...
| `hidden` | | `true` → тур **полностью исчезает** с главной: нет карточки, не может стать «ближайшим». Ставится во всех трёх языках. Страница `tours/<id>.html` всё равно генерируется и доступна по прямой ссылке |
...
Карточка на главной и текст тикера берутся из одной функции `findNearestDeparture(c)` в main.js.
Она идёт по short + long, берёт самую раннюю дату и **пропускает**: прошедшие выезды, `soldOut: true`, `hidden: true` и `active: false`.
Значит, чтобы сменить «ближайший тур», править нужно только TOUR_DATES, не трогая тексты.
...
- **Тур в тикере не вбивается руками.** `ticker` — шаблон с плейсхолдерами `{tour}` и `{dates}`, которые подставляет `renderTicker()`
